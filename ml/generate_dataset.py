...
def generate_synthetic_dataset(num_samples: int = 50000, random_state: int = 42) -> pd.DataFrame:
    ...
    historical_payment_success = np.clip(np.random.beta(a=8, b=2, size=num_samples), 0.3, 0.99)
    ...
    transaction_history = np.random.randint(1, 100, size=num_samples)

    # Calculate log-odds score for recovery probability
    log_odds = 0.5

    # Positive factors (High historical success + long tenure + temporary failure)
    log_odds += (historical_payment_success - 0.7) * 3.5
    log_odds += (engagement_score - 0.5) * 2.0
    log_odds += np.where(tenure_months > 12, 0.6, -0.2)
    log_odds += np.where(transaction_history > 20, 0.4, 0.0)
    ...
    df = pd.DataFrame({
        ...
        "historical_payment_success": historical_payment_success,
        "historical_payment_success_rate": historical_payment_success,
